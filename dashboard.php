...
<script>
    ...
    $sql = "
       ...
       SUM(finance_fees.balance) balance, finance_fee_collections.start_date start_date, finance_fee_collections.due_date due_date,

       ROUND(
           (
               ((SUM(finance_fees.particular_total) - (IF(SUM(finance_fee_discounts.discount_amount) is null, 0,
                                                         SUM(finance_fee_discounts.discount_amount)))) -
               SUM(finance_fees.balance))
                   /
               ((SUM(finance_fees.particular_total) -
                (IF(SUM(finance_fee_discounts.discount_amount) is null, 0, SUM(finance_fee_discounts.discount_amount)))))
           ) * 100,2) Percentage
...
WHERE STR_TO_DATE(finance_fee_collections.start_date, '%Y-%m-%d') >= '2019-09-01'
...
GROUP BY courses.course_name;
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            array_push($fees, $row['Percentage']);
