...
$student_sql = "
SELECT 
       student_parent_info.familyid, parent,student,admission_no,contact_no,contact_no1,$grade_children,
...
FROM (
      (SELECT s.id                                         sid,
              ...
              (SELECT CONCAT(mobile_phone, ' ',office_phone1) from guardians where s.immediate_contact_id = id) 'contact_no1'
       FROM students s
                INNER JOIN guardians g on s.familyid = g.familyid
                INNER JOIN batches b on s.batch_id = b.id
                INNER JOIN courses c on b.course_id = c.id
       WHERE (s.is_active = 1) AND g.relation = 'Staff'
          $group
      ...
      LEFT JOIN (SELECT s.id     sid,s.familyid,
                  ...
                  WHERE (ffc.is_deleted = 0 $condition) 
                    $group 
          ) as current_fees ON $join
    
         LEFT JOIN (SELECT IFNULL(SUM(balance),'0') opening_balance, s.familyid, s.id sid
                    FROM `finance_fees` ff
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if (($row['balance'] > 0) or ($row['opening_balance'] > 0)) {
            ...
            if (preg_replace('/\s+/', '', $row['contact_no']) == '' )
                echo $row['contact_no1'];
            else
                echo $row['contact_no'];
